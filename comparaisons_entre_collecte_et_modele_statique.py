"""
Comparer les psitions de la collecte avec les positions calculées (minimisation énergie) à l'aide des k obtenus par optim
K tirés de Jacques (2008)
Possibilité de changer les k pour vérifier résultats de l'optim statique
"""
...
participant = 0
nb_disques = 7
statique_name = 'labeled_statique_leftfront_D' + str(nb_disques)
...
force=1
position = 1
instant_collecte = 700

def Resultat_simu (k_type, ind_masse, Masse_centre) :
    masse_type = 'repartie' #'ponctuelle'
    ...
    def rotation_points(Pos_repos, Pt_ancrage):
        ...
        for index in range(Nb_ressorts_cadre):
            Pt_ancrage_new[index, :] = np.matmul(Pt_ancrage[index, :],mat_base_collecte)

        Pos_repos_new = np.zeros((n * m, 3))
        for index in range(n * m):
            Pos_repos_new[index, :] = np.matmul(Pos_repos[index, :],mat_base_collecte)
    ...
    def Energie_func(Pt, Pt_ancrage, k, M, l_repos, k_croix, l_repos_croix):
        ...
        for i in range(135):
            Energie += M[i].T * 9.81 * Pt[2, i]

        func = cas.Function('Energie_func', [Pt], [Energie]).expand()

        return func

    def Optimisation_toile(k, k_croix, Pt_ancrage, Pos_repos, ind_masse, Masse_centre, dict_fixed_params):

        M = cas.MX(dict_fixed_params["M"])
        l_repos = cas.MX(dict_fixed_params["l_repos"])
        l_repos_croix = cas.MX(dict_fixed_params["l_repos_croix"])
    ...
    k, k_croix = Param_variable(k_type)
    ###################################################################################################################

    Pt = Optimisation_toile(k, k_croix, Pt_ancrage, Pos_repos, ind_masse, Masse_centre, dict_fixed_params)
    return Pt, Pt_ancrage

def Resultat_PF_collecte(participant,statique_name, vide_name) :
    ...
    def plateforme_calcul(platform):  # prend les plateformes separees, passe les N en Nmm, calibre, multiplie par mat rotation, met dans la bonne orientation
        ...
        #diviser par 4 le nombre de points pour avoir même fréquence que les caméras
    ...
    label_min = point_le_plus_bas(Pt_collecte, labels, instant_collecte) #coordonnée du marqueur le plus bas dans labels
    ind_marqueur_min = int(label_min[1:3]) #coordonnées de ce marqueur adaptées à la simulation

    return F_totale_collecte,Pt_collecte,labels,ind_marqueur_min
...
def k_type_initiaux() :
    ...
    k_type = [k1, k2, k3, k4, k5, k6, k7, k8, k_oblique1, k_oblique2, k_oblique3, k_oblique4]
    return k_type


# résultats de la collecte (fixe)
F_totale_collecte,Pt_collecte,labels,ind_marqueur_min = Resultat_PF_collecte(participant,statique_name, vide_name)
ind_masse = ind_marqueur_min
nom_masse = 't' + str(ind_masse)

# résultat de la simu :
# k_type = k_type_initiaux() #changeable
k_type = [1.92152820e+03, 5.99287710e+04, 6.04691062e+01, 4.56981051e+04,
 2.64137450e+04, 3.32897021e+05, 2.68656632e+04, 1.07895937e+05,
 1.10664316e+03, 1.94556727e+05, 1.34126659e+05, 4.62782913e+05]
Masse_centre = masses[nb_disques]
print('Raideurs des ressorts : ' + str(k_type))
print('Masse de ' + str(Masse_centre) + 'kg appliquée au point ' + str(ind_masse))
Pt_simu, Pt_ancrage_simu= Resultat_simu(k_type, ind_masse, Masse_centre)
...
#Affichage
ind_masse_collecte = labels.index(nom_masse)
ind_masse_simu = ind_masse
...
if force == 1 :
    ...
    for i in range (3) :
        plt.subplot(3,1,i+1)
        plt.plot(F_totale_collecte[:,i],'-b')
        plt.ylabel(legende[i])
        if i == 0 :
            plt.title(('Somme des forces mesurées en chaque plateforme'))
...
########################################################################################################################
    ...
    for i in range(3):
        plt.subplot(3, 1, i + 1)
        plt.plot(Pt_collecte[i, ind_masse_collecte, :], '-b')
        plt.ylabel(legende[i])
        if i == 0:
            plt.title(('Position mesurée du point ' + nom_masse))
...
# ########################################################################################################################
    ...
    ax.plot(Pt_collecte[0,:,instant_collecte],Pt_collecte[1,:,instant_collecte],Pt_collecte[2,:,instant_collecte],'.b')
    ax.plot(Pt_collecte[0,ind_masse_collecte,instant_collecte],Pt_collecte[1,ind_masse_collecte,instant_collecte],Pt_collecte[2,ind_masse_collecte,instant_collecte],'.b')
    plt.title('Points mesurés à l\'instant ' + str(instant_collecte))
#
# ########################################################################################################################
    fig = plt.figure(23)
    ax = fig.add_subplot(1, 1, 1, projection='3d')
    ax.set_box_aspect([1.1, 1.8, 1])
    ax.plot(Pt_simu.T[ :, 0], Pt_simu.T[ :, 1], Pt_simu.T[ :, 2], '.r')
    ax.plot(Pt_simu.T[ ind_masse_simu, 0], Pt_simu.T[ ind_masse_simu, 1],
            Pt_simu.T[ ind_masse_simu, 2], '.b')
    ax.plot(Pt_simu.T[ ind_masse_simu+1, 0], Pt_simu.T[ ind_masse_simu+1, 1],
            Pt_simu.T[ ind_masse_simu+1, 2], '.g')
    ax.plot(Pt_simu.T[ ind_masse_simu-1, 0], Pt_simu.T[ ind_masse_simu-1, 1],
            Pt_simu.T[ ind_masse_simu-1, 2], '.g')
    ax.plot(Pt_simu.T[ ind_masse_simu+15, 0], Pt_simu.T[ ind_masse_simu+15, 1],
            Pt_simu.T[ ind_masse_simu+15, 2], '.g')
    ax.plot(Pt_simu.T[ ind_masse_simu-15, 0], Pt_simu.T[ ind_masse_simu-15, 1],
            Pt_simu.T[ ind_masse_simu-15, 2], '.g')
    ax.plot(Pt_ancrage_simu[ :, 0], Pt_ancrage_simu[ :, 1],
            Pt_ancrage_simu[ :, 2], '.k')
    plt.title('Points simulés en statique (min. énergie)')

########################################################################################################################
    fig = plt.figure(24)
    ax = fig.add_subplot(1, 1, 1, projection='3d')
    ax.set_box_aspect([1.1, 1.8, 1])
    ax.plot(Pt_simu.T[ :, 0], Pt_simu.T[ :, 1], Pt_simu.T[ :, 2], '.b',label = 'points simulés')
    ax.plot(Pt_simu.T[ ind_masse_simu, 0], Pt_simu.T[ ind_masse_simu, 1],
            Pt_simu.T[ ind_masse_simu, 2], '.y',label = 'point simulé où est appliquée la masse')
    ax.plot(Pt_ancrage_simu[ :, 0], Pt_ancrage_simu[:, 1],
            Pt_ancrage_simu[ :, 2], '.b')
    ax.plot(Pt_collecte[0, :, instant_collecte], Pt_collecte[1, :, instant_collecte],
           Pt_collecte[2, :, instant_collecte], 'xr',label = 'points mesurés')
    ax.plot(Pt_collecte[0, ind_masse_collecte, instant_collecte],
            Pt_collecte[1, ind_masse_collecte, instant_collecte],
            Pt_collecte[2, ind_masse_collecte, instant_collecte], 'xm',label = 'point mesuré où est appliquée la masse')
    plt.legend()

    plt.title('Comparaison 3D entre les points simulés  et mesurés \n à l\'instant '+ str(instant_collecte) + ' pour l\'essai ' + str(statique_name))

########################################################################################################################
    plt.figure(25)
    plt.plot(Pt_simu.T[ :, 0], Pt_simu.T[ :, 1], '.r',label = 'points simulés')
    plt.plot(Pt_simu.T[ ind_masse_simu, 0], Pt_simu.T[ ind_masse_simu, 1],
             '.y',label = 'point simulé où est appliquée la masse')
    # plt.plot(Pt_ancrage_simu[ :, 0], Pt_ancrage_simu[ :, 1], '.r')
    plt.plot(Pt_collecte[0, :, instant_collecte], Pt_collecte[1, :, instant_collecte],
            '.b',label = 'points mesurés')
    plt.plot(Pt_collecte[0, ind_masse_collecte, instant_collecte],
            Pt_collecte[1, ind_masse_collecte, instant_collecte],
             '.c',label = 'point mesuré où est appliquée la masse')
    plt.title('Comparaison 2D entre les points simulés en statique, et mesurés à l\'instant '+ str(instant_collecte))
    plt.legend()
...
for ind in range (135) :
    if 't' + str(ind) in labels:
        ...
        for i in range (3) :
            if np.isnan(Pt_collecte[i, ind_collecte, instant_collecte]) == False :  # gérer les nans
                erreur_position += (Pt_simu.T[ind, i] - Pt_collecte[i, ind_collecte, instant_collecte]) ** 2

print('*********************************************************************')
print('Erreur de position : ' + str(np.sqrt(erreur_position)) + ' m')
