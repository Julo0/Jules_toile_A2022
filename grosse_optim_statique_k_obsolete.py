"""
Optimisation des k en single shooting
...
def Resultat_PF_collecte(participant, vide_name, trial_name, frame) :
    ...
    def Named_markers(c3d_experimental):
        ...
        else : #certains essais n'ont aucun point non labellé
            labels = c3d_experimental['parameters']['POINT']['LABELS']['value'][:]
            named_positions = c3d_experimental['data']['points'][0:3, :, :]
...
def interpolation_collecte(Pt_collecte, Pt_ancrage, labels) :
    ...
    # ax.plot(Pt_coll[0, :], Pt_coll[1, :], Pt_coll[2, :], '.b', label='Points collecte')
    #
    # plt.legend()
    # plt.title('Verif interpolation')
    # plt.show()
...
def Calcul_Pt_F(X, Pt_ancrage, dict_fixed_params, K) :
    ...
    F_spring, F_spring_croix, F_masses = Force_calc(Spring_bout_1, Spring_bout_2, Spring_bout_croix_1,
                                                    Spring_bout_croix_2, k, k_croix, dict_fixed_params)
...
def a_minimiser(X, K, F_totale_collecte, Pt_collecte, Pt_ancrage, dict_fixed_params, labels, min_energie):

    F_totale, F_point = Calcul_Pt_F(X, Pt_ancrage, dict_fixed_params, K)
    Pt = list2tab(X)
...
def Optimisation(participant, Masse_centre, trial_name, vide_name, frame, initial_guess, min_energie) :  # main
    ...
    def Pt_bounds_repos(Pos, Masse_centre) :
        ...
        # F = np.array([0,0,Masse_centre*9.81])
        # for i in range (3) :
        #     lbw_Pt = [F[i] - 100]
        #     ubw_Pt = [F[i] + 100]
        #     w0_Pt = [F[i]]
    ...
    J = a_minimiser(X, K, F_totale_collecte, Pt_collecte, Pt_ancrage,dict_fixed_params,labels,min_energie)
    obj = J(X,K)

    # Create an NLP solver
    prob = {'f': obj, 'x': cas.vertcat(*w)}#, 'g': cas.vertcat(*g)} #pas de fonction contrainte ici
    ...
    #        ubx=cas.vertcat(*ubw))
    sol = solver(x0=cas.vertcat(*w0), lbx=cas.vertcat(*lbw),ubx=cas.vertcat(*ubw))
    w_opt = sol['x'].full().flatten()

    return w_opt, Pt_collecte, F_totale_collecte, ind_masse, labels, Pt_ancrage, dict_fixed_params

def Affichage_resultat(Pt_collecte, Pt_ancrage, Pt, ind_masse, labels, trial_name) :
    ...
    ax.plot(Pt[ind_masse, 0], Pt[ind_masse, 1], Pt[ind_masse, 2], '.y',
    ...
    ax.plot(Pt_collecte[0, label_masse], Pt_collecte[1, label_masse], Pt_collecte[2, label_masse], 'xm',
            label='Point collecte le plus bas ' + labels[label_masse])
    plt.legend()
    plt.title('Comparaison de ' + trial_name + ' avec les point optimisés')
    ax.set_xlabel('x (m)')
    ax.set_ylabel('y (m)')
    ax.set_zlabel('z (m)')
...
#Liste des essais à optimiser :
essais = []
participants = []
for i in range (1, 12) :
    essais+= ['labeled_statique_D' + str(i)]
    participants+= [0]
for i in range (1, 12) :
    essais+= ['labeled_statique_left_D' + str(i)]
    participants+= [0]
for i in range (1, 12) :
    essais+= ['labeled_statique_centrefront_D' + str(i)]
    participants+= [0]
for i in range (1, 12) :
    essais+= ['labeled_statique_leftfront_D' + str(i)]
    participants+= [0]
essais+=['labeled_p2_statique_01']
participants+= [2]
essais+=['labeled_p2_statique_02']
participants+= [2]
essais+=['labeled_p1_statique_02_pfmauvais']
participants+= [1]
essais+=['labeled_p1_statique_03_pfmauvais']
participants+= [1]
...
for i in range (14,15) :
    ...
    # F_out_cas,Pt_out_cas,labels,ind_marqueur_min = Resultat_PF_collecte(participant, vide_name, trial_name, frame)


    Solution, Pt_collecte, F_totale_collecte, ind_masse, labels, Pt_ancrage, dict_fixed_params = Optimisation(
        participant, Masse_centre, trial_name, vide_name, frame, initial_guess, min_energie)

    k=np.array(Solution[:12])
    Pt = np.reshape(Solution[12:],(135,3))
    print('k = ' +str(k))

    # print('**************************************************************************')
    np.save('k_grosse_optim_statique_1808_' + trial_name + '.npy', k)
    np.save('points_grosse_optim_statique_1808_' + trial_name + '.npy', Pt)
    Affichage_resultat(Pt_collecte, Pt_ancrage, Pt, ind_masse, labels, trial_name)
    print('*****************************************************************')
