...
def rotation_points (Pos_repos,Pt_ancrage) :
    ...
    for index in range (Nb_ressorts_cadre) :
        Pt_ancrage_new[index,:] = np.matmul(Pt_ancrage[index,:], mat_base_collecte) #multplication de matrices en casadi

    Pos_repos_new = np.zeros((n*m, 3))
    for index in range(n*m):
        Pos_repos_new[index, :] = np.matmul(Pos_repos[index, :], mat_base_collecte)  # multplication de matrices en casadi
...
def Calcul_Pt_F_verif(X, Pt_ancrage, dict_fixed_params, K) :
    k, k_croix = Param_variable(K)
    Pt = list2tab(X)
...
def Optimisation(participant, Masse_centre, trial_name, vide_name, frame, initial_guess, min_energie) :  # main
    ...
    # Solve the NLP
    sol = solver(x0=cas.vertcat(*w0), lbx=cas.vertcat(*lbw),ubx=cas.vertcat(*ubw)) #pas de fonction contrainte
...
frame = 700
participant = 0 #0 #1 #2
nb_disques = 7 #entre 1 et 11
# trial_name = 'labeled_statique_leftfront_D9'
trial_name = 'labeled_statique_leftfront_D' +str(nb_disques)
...
F_totale, F_point = Calcul_Pt_F_verif(Solution[12:], Pt_ancrage, dict_fixed_params, Solution[:12])
...
np.save('k_optim_statique_' + trial_name + '.npy', k)
np.save('points_optim_statique_' + trial_name + '.npy', Pt)
...
ax.plot(Pt[ind_masse, 0], Pt[ind_masse, 1], Pt[ind_masse, 2], '.y', label='Point optimisé le plus bas d\'indice ' + str(ind_masse))
...
print('Erreur sur la position : ' +str(np.sqrt(erreur_position)) + ' m')
print('Erreur sur la force : ' +str(np.sqrt(erreur_force)) + ' N')
