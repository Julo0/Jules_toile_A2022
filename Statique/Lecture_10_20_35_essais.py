...
/!\ je fais le choix de supprimer la contrainte sur Force_equilibre_func
...
def Force_calc(Spring_bout_1, Spring_bout_2, Spring_bout_croix_1, Spring_bout_croix_2, k,k_oblique, M, dict_fixed_params, ind_masse):  # force dans chaque ressort
    """
    ...
    #force des disques en i, i+-1 et i+-15
...
path = 'results/result_multi_essais/' + 'optim_sur_35_essais_mediane' + '.pkl'#mmmmmm
with open(path, 'rb') as file:
    ...
    erreur_position = pickle.load(file)
    erreur_force = pickle.load(file)
    f = pickle.load(file)
    dict_fixed_params = pickle.load(file)
    trial_name = pickle.load(file)
print(Solution[:12])
...
for i in range(0,nb_essais):
    ...
    F_totale.append(Calcul_Pt_F_verif(Solution[17+405*i+5*i : 422+405*i+5*i], Pt_ancrage[i], dict_fixed_params, k, ind_masse[i], Solution[12+405*i+5*i : 17+405*i+5*i])[0])
    F_point.append(Calcul_Pt_F_verif(Solution[17+405*i+5*i : 422+405*i+5*i], Pt_ancrage[i], dict_fixed_params, k, ind_masse[i], Solution[12+405*i+5*i : 17+405*i+5*i])[1])
...
for i in range (0,nb_essais):
    ...
    plt.title(str(trial_name[i]), fontsize=8)
    ax.set_xlabel('x (m)')
    ax.set_ylabel('y (m)')
    ax.set_zlabel('z (m)')
# plt.legend()
# plt.legend(bbox_to_anchor=(-10,6), loc='center')
...
for p in range (0,nb_essais):
    ...
    for ind in range (n*m) :
        for i in range (3) :
            err_for += (F_point[p][ind, i]) ** 2
    erreur_force.append(err_for)
