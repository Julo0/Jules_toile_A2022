...
On load 10 fichiers c3d pour avoir les informations des essai, puis a partir de ca on définit tous les parametres fixes du trampoline.
...
L'objectif :
-minimiser la résultante des forces en chacun des 135 points des 10 essais
...
Les 10 essais sont optimisés simultanement

L'optimisation renvoie alors :
-la valeur des k et des 5 masses optimisées pour les 10 essais
-les coordonnées des 135 points des 10 essais
-le label du point sur lequel le disque est posé pour chaque essai
...
...
nb_disques = [3,4,9,4,8,1,6,7,3,5] #choix des 2 masses
#premier essai
frame = 700
...
essais+= ['labeled_statique_leftfront_D' + str(nb_disques[3])]
essais+= ['labeled_statique_left_D' + str(nb_disques[4])]
essais+= ['labeled_statique_centrefront_D' + str(nb_disques[5])]
essais+= ['labeled_statique_left_D' + str(nb_disques[6])]
essais+= ['labeled_statique_D' + str(nb_disques[7])]
essais+= ['labeled_statique_left_D' + str(nb_disques[8])]
essais+= ['labeled_statique_leftfront_D' + str(nb_disques[9])]
...
for i in range(0,10):
    ...
    Pt_collecte[i] = np.array(Pt_collecte[i])
    Pt_ancrage[i] = np.array(Pt_ancrage[i])
...
# path = 'results/result_multi_essais/' + 'optim_sur_10_essais_corr1' + '.pkl'
