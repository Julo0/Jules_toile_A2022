...
On load 2 fichiers c3d pour avoir les informations des essai, puis a partir de ca on définit tous les parametres fixes du trampoline.
...
L'objectif :
-minimiser la résultante des forces en chacun des 135 points des 2 essais
-minimiser la différence entre la position des points de collecte et la position du modele
Les contraintes :
-la somme des 5 masses appliquée au niveau des points en contact avec le disque est égale a la masse du disque

Les deux essais sont optimisés simultanement

L'optimisation renvoie alors :
-la valeur des k et des 5 masses optimisées pour les 2 essais
-les coordonnées des 135 points des 2 essais
...
def Optimisation(participant, Masse_centre, trial_name, vide_name, frame, initial_guess, min_energie) :  # main
    ...
    def k_bounds () : #initial guess pour les k et les C
        ...
        :return:
        """
        k1 = 24828.076927527232  # un type de coin (ressort horizontal)
        k2 = 40856.10796502957 # ressorts horizontaux du bord (bord vertical) : relient le cadre et la toile
        k3 = 2650.206028030497  # un type de coin (ressort vertical)
        k4 = 27577.87146126246  # ressorts verticaux du bord (bord horizontal) : relient le cadre et la toile
        k5 = 121368.42130004517  # ressorts horizontaux du bord horizontal de la toile
        k6 = 95656.99556602963   # ressorts horizontaux
        k7 = 60472.10098284219   # ressorts verticaux du bord vertical de la toile
        k8 = 122337.565402068  # ressorts verticaux
        # VALEURS INVENTÉES :
        k_oblique1 = 42112.98660292059  # 4 ressorts des coins
        k_oblique2 = 88836.05803177971   # ressorts des bords verticaux
        k_oblique3 = 124825.33879620211   # ressorts des bords horizontaux
        k_oblique4 = 273928.4520848462   # ressorts obliques quelconques
...
essais = []
participants = [0,0]
nb_disques = [9,4] #choix des 2 masses
#premier essai
frame = 700
essais+= ['labeled_statique_centrefront_D' + str(nb_disques[0])]
essais+= ['labeled_statique_left_D' + str(nb_disques[1])]
participant = []           #creation d une liste pour gerer les participants
trial_name = []             #creation d une liste pour gerer les essais
vide_name = ['labeled_statique_centrefront_vide', 'labeled_statique_vide']              #creation d une liste pour les essais a vide
Masse_centre = []
...
#ESSAI 1
F_totale1, F_point1 = Calcul_Pt_F_verif(Solution[17:422], Pt_ancrage[0], dict_fixed_params, Solution[:12], ind_masse[0],Solution[12:17])
F_totale1 = cas.evalf(F_totale1)
F_point1 = cas.evalf(F_point1)
F_point1 = np.array(F_point1)
labels1 = labels[0]
#ESSAI 2
F_totale2, F_point2 = Calcul_Pt_F_verif(Solution[427:832], Pt_ancrage[1], dict_fixed_params, Solution[:12], ind_masse[1],Solution[422:427])
...
ax.set_box_aspect([1.1, 1.8, 1])
ax.plot(Pt1[:, 0], Pt1[:, 1], Pt1[:, 2], '+r', label = 'Points de la toile optimisés')
ax.plot(Pt_ancrage1[:, 0], Pt_ancrage1[:, 1], Pt_ancrage1[:, 2], '.k', label = 'Points d\'ancrage simulés')
ax.plot(Pt1[ind_masse[0], 0], Pt1[ind_masse[0], 1], Pt1[ind_masse[0], 2], '+y', label='Point optimisés le plus bas d\'indice ' + str(ind_masse[0]))
ax.plot(Pt_collecte1[0, :], Pt_collecte1[1, :], Pt_collecte1[2, :], '.b', label = 'Points collecte')
label_masse = labels1.index('t' + str(ind_masse[0]))
ax.plot(Pt_collecte1[0, label_masse], Pt_collecte1[1, label_masse], Pt_collecte1[2, label_masse], 'og', label = 'Point collecte le plus bas ' + labels1[label_masse])
plt.legend()
#plt.title('Essai d\'optimisation améliorée : \n Comparaison de l\'essai statique ' + trial_name + ' \n avec les positions optimisées')
plt.title('Fusion optim ' + str(trial_name[0]))
ax.set_xlabel('x (m)')
ax.set_ylabel('y (m)')
ax.set_zlabel('z (m)')
...
ax.set_box_aspect([1.1, 1.8, 1])
ax.plot(Pt2[:, 0], Pt2[:, 1], Pt2[:, 2], '+r', label = 'Points de la toile optimisés')
ax.plot(Pt_ancrage2[:, 0], Pt_ancrage2[:, 1], Pt_ancrage2[:, 2], '.k', label = 'Points d\'ancrage simulés')
ax.plot(Pt2[ind_masse[1], 0], Pt2[ind_masse[1], 1], Pt2[ind_masse[1], 2], '+y', label='Point optimisés le plus bas d\'indice ' + str(ind_masse[1]))
ax.plot(Pt_collecte2[0, :], Pt_collecte2[1, :], Pt_collecte2[2, :], '.b', label = 'Points collecte')
label_masse = labels2.index('t' + str(ind_masse[1]))
ax.plot(Pt_collecte2[0, label_masse], Pt_collecte2[1, label_masse], Pt_collecte2[2, label_masse], 'og', label = 'Point collecte le plus bas ' + labels2[label_masse])
#plt.legend()
#plt.title('Essai d\'optimisation améliorée : \n Comparaison de l\'essai statique ' + trial_name + ' \n avec les positions optimisées')
plt.title('Fusion optim ' + str(trial_name[1]))
ax.set_xlabel('x (m)')
ax.set_ylabel('y (m)')
ax.set_zlabel('z (m)')
...
for ind in range (2*n*m) :
    if 't' + str(ind) in labels1:
        ind_collecte1 = labels1.index('t' + str(ind))  # ATTENTION gérer les nans
        for i in range (3) :
            if np.isnan(Pt_collecte1[i, ind_collecte1]) == False :  # gérer les nans
                erreur_position1 += (Pt1[ind, i] - Pt_collecte1[i, ind_collecte1]) ** 2
    if 't' + str(ind) in labels2:
        ind_collecte2 = labels2.index('t' + str(ind))
        for i in range(3):
            if np.isnan(Pt_collecte2[i, ind_collecte2]) == False :  # gérer les nans
                erreur_position2 += (Pt2[ind, i] - Pt_collecte2[i, ind_collecte2]) ** 2

erreur_force1 = 0
erreur_force2 = 0
for ind in range (n*m) :
    for i in range (3) :
        erreur_force1 += (F_point1[ind, i]) ** 2
        erreur_force2 += (F_point2[ind, i]) ** 2


print('-Erreur sur la position-  ' + str(trial_name[0]) + ' = ' + str(erreur_position1) + ' m' + ' // ' + str(trial_name[1]) + ' = ' + str(erreur_position2) + ' m')
print('-Erreur sur la force-  ' + str(trial_name[0]) + ' = ' + str(erreur_force1) + ' N' + ' // ' + str(trial_name[1]) + ' = ' + str(erreur_force2) + ' N')


#ENREGISTREMENT PICKLE#
path = 'results/result_2essais/' + str(trial_name[0]) + '_' + str(trial_name[1]) + '.pkl'
with open(path, 'wb') as file:
    ...
    pickle.dump(erreur_position1, file)
    pickle.dump(erreur_position2, file)
    pickle.dump(erreur_force1, file)
    pickle.dump(erreur_force2, file)
    pickle.dump(f, file)
