...
On load un fichier c3d pour avoir les informations d'un essai, puis a partir de ca on définit tous les parametres fixes du trampoline.
...
######################################################################################################################
    ...
    mpetitbord = 0.5 * (Mtrampo / ((n-1)*(m-1)) ) + 2*((mressort_bord/2) + mattache)        #masse d un point situé sur le petit bord
    mgrandbord = 0.5 * (Mtrampo / ((n - 1) * (m - 1))) + (33/13) * ((mressort_bord / 2) + mattache)     #masse d un point situé sur le grand bord
    mcoin = 0.25 * (Mtrampo / ((n - 1) * (m - 1))) + mressort_coin + 4*((mressort_bord / 2) + mattache)     #masse d un point situé dans un coin
...
def Force_calc(Spring_bout_1, Spring_bout_2, Spring_bout_croix_1, Spring_bout_croix_2, k,k_oblique, M, dict_fixed_params, ind_masse):  # force dans chaque ressort
    ...
    for i in range(Nb_ressorts_croix):
        Vect_unit_dir_F_croix[i,:] = (Spring_bout_croix_2[i,:] - Spring_bout_croix_1[i,:]) / cas.norm_fro(Spring_bout_croix_2[i,:] - Spring_bout_croix_1[i,:])
...
def main():
    ...
    Pt = np.reshape(Solution[17:],(135,3))
    print ('k = ' +str(k))
    print('M = ' + str(M))
    end_main = time.time()
    ...
    F_totale, F_point = Calcul_Pt_F_verif(Solution[17:], Pt_ancrage, dict_fixed_params, Solution[:17], ind_masse)
    ...
    path = 'results/result_1essai_diff*1000/' + str(trial_name) + '.pkl'
