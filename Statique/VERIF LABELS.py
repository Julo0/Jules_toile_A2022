...
#fonction contrainte :
    ...
    opts = {"ipopt": {"max_iter" :1, "linear_solver":"ma57"}}
...
for i in range (len(labels)):
   ax.text(Pt_collecte[0, i], Pt_collecte[1, i], Pt_collecte[2, i], labels[i], color='black')
...
for ind in range (n*m) :
    for i in range (3) :
        force_ressort.append(F_point[ind, i])
...
print('forces de chaque ressort' + str(force_ressort))

# for j in range(n*m):
#     # pas tres elegant mais cest le seul moyen pour que ca fonctionne
#     a = []
#     a = np.append(a, Pt[j, 0])
#     a = np.append(a, Pt[j, 0] + F_point[j, 0]/100)
#
#     b = []
#     b = np.append(b, Pt[j, 1])
#     b = np.append(b, Pt[j, 1] + F_point[j, 1]/100)
#
#     c = []
#     c = np.append(c, Pt[j, 2])
#     c = np.append(c, Pt[j, 2] + F_point[j, 2]/100)
#
