...
######################################################################################################################
    ...
    M[ind_masse] += k_type[8]
    M[ind_masse + 1] += k_type[9]
    M[ind_masse - 1] += k_type[10]
    M[ind_masse + 15] += k_type[11]
    M[ind_masse - 15] += k_type[12]
...
def Force_calc(Spring_bout_1, Spring_bout_2, k, M, dict_fixed_params, ind_masse):  # force dans chaque ressort
    """
    ...
    for i in range(Nb_ressorts):
        Vect_unit_dir_F[i,:] = (Spring_bout_2[i,:] - Spring_bout_1[i,:]) / cas.norm_fro(Spring_bout_2[i,:] - Spring_bout_1[i,:])
    ...
    return F_spring, F_masses

def Force_point(F_spring, F_masses):  # --> resultante des forces en chaque point a un instant donne
    """
    ...
    F_spring_points[0, :] = F_spring[0, :] + \
                            ...
                            F_spring[Nb_ressorts_cadre + Nb_ressorts_horz, :]  # en bas a droite : premier ressort du cadre + dernier ressort du cadre + premiers ressorts horz, vert et croix
...
def Calcul_Pt_F_verif(X, Pt_ancrage, dict_fixed_params, K, ind_masse) :
    k, M = Param_variable(K, ind_masse)
    Pt = list2tab(X)

    Spring_bout_1, Spring_bout_2 = Spring_bouts(Pt, Pt_ancrage)
    F_spring, F_masses = Force_calc(Spring_bout_1, Spring_bout_2,k, M, dict_fixed_params, ind_masse)
    F_point = Force_point(F_spring, F_masses)
...
def a_minimiser(X, K, F_totale_collecte, Pt_collecte, Pt_ancrage, dict_fixed_params, labels, min_energie, ind_masse):
    ...
    F_totale, F_point = Calcul_Pt_F_verif(X, Pt_ancrage, dict_fixed_params, K, ind_masse)
    Pt = list2tab(X)
...
def longueur_ressort(dict_fixed_params, Pt, Pt_ancrage) :
    ...
    delta = cas.MX.zeros(Nb_ressorts)
    for i in range (Nb_ressorts) :
...
def Optimisation(participant, Masse_centre, trial_name, vide_name, frame, initial_guess, min_energie) :  # main
    ...
    def k_bounds () : #initial guess pour les k et les m
        """
        ...
        w0_k = [k1, k2, k3, k4, k5, k6, k7, k8, M1, M2, M3, M4, M5]
        for i in range (len(w0_k)) :
            w0_k[i] = 1*w0_k[i]

        lbw_k = [1e-3]*8
        lbw_k += [0.4*Masse_centre/5]*5
        ubw_k = [1e7]*8 # bornes très larges
        ubw_k += [1.6*Masse_centre/5]*5
...
#fonction contrainte :
    g += [K[8] +  K[9] + K[10] + K[11] + K[12] - Masse_centre]
    lbg += [0]
    ubg += [0]
...
Pt = np.reshape(Solution[13:],(135,3))
print ('k = ' +str(k))
print('M = ' + str(M))
end_main = time.time()
...
F_totale, F_point = Calcul_Pt_F_verif(Solution[13:], Pt_ancrage, dict_fixed_params, Solution[:13], ind_masse)
F_totale = cas.evalf(F_totale)
...
ax.plot(Pt_collecte[0, label_masse], Pt_collecte[1, label_masse], Pt_collecte[2, label_masse], '.g', label = 'Point collecte le plus bas ' + labels[label_masse])
plt.legend()
plt.title('Ressort structurel uniquement  ' + trial_name + ' \n avec les positions optimisées')
ax.set_xlabel('x (m)')
ax.set_ylabel('y (m)')
ax.set_zlabel('z (m)')
...
path = 'results/result_1essai_ressort_que_structurel/' + str(trial_name) + '.pkl'
