...
def Resultat_PF_collecte(participant, vide_name, trial_name, frame) :
    ...
    def open_c3d(participant, trial_name):
      ...
      """
        ...
        file_path = '../Collecte/statique_100%' #+ dossiers[participant]
...
def Optimisation(participant, Masse_centre, trial_name, vide_name, frame, initial_guess, min_energie) :  # main
    ...
    def k_bounds () : #initial guess pour les k a partir des donnees issues des simulations pour essai individuel
        """
...
nb_disques = [1,2,3,6,7,8,9,10,11]+[1,3,4,5,6,8,9,10,11]+[1,4,5,6,7,8,9,10,11]+[2,3,5,6,7,8,9,10,11]
...
for i in range (0,9):
    ...
    F_totale_centrefront.append(Calcul_Pt_F(Solution[17+405*i+5*i:422+405*i+5*i], Pt_ancrage[i], dict_fixed_params, Solution[:12], ind_masse[i],Solution[12+405*i+5*i:17+405*i+5*i])[0])
    F_point_centrefront.append(Calcul_Pt_F(Solution[17+405*i+5*i:422+405*i+5*i], Pt_ancrage[i], dict_fixed_params, Solution[:12], ind_masse[i],Solution[12+405*i+5*i:17+405*i+5*i])[1])
...
for i in range(9, 18):
    ...
    F_totale_statique.append(
        Calcul_Pt_F(Solution[17 + 405 * i + 5 * i:422 + 405 * i + 5 * i], Pt_ancrage[i], dict_fixed_params,
...
for i in range (18,27):
    ...
    F_totale_leftfront.append(Calcul_Pt_F(Solution[17+405*i+5*i:422+405*i+5*i], Pt_ancrage[i], dict_fixed_params, Solution[:12], ind_masse[i],Solution[12+405*i+5*i:17+405*i+5*i])[0])
    F_point_leftfront.append(Calcul_Pt_F(Solution[17+405*i+5*i:422+405*i+5*i], Pt_ancrage[i], dict_fixed_params, Solution[:12], ind_masse[i],Solution[12+405*i+5*i:17+405*i+5*i])[1])
...
for i in range (27,36):
    ...
    F_totale_left.append(Calcul_Pt_F(Solution[17+405*i+5*i:422+405*i+5*i], Pt_ancrage[i], dict_fixed_params, Solution[:12], ind_masse[i],Solution[12+405*i+5*i:17+405*i+5*i])[0])
    F_point_left.append(Calcul_Pt_F(Solution[17+405*i+5*i:422+405*i+5*i], Pt_ancrage[i], dict_fixed_params, Solution[:12], ind_masse[i],Solution[12+405*i+5*i:17+405*i+5*i])[1])
